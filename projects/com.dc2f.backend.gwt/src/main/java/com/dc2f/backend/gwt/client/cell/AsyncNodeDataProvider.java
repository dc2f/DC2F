...
public class AsyncNodeDataProvider extends AsyncDataProvider<Node> {
	...
	public AsyncNodeDataProvider(DC2FNavigationServiceAsync navigationService, Node value) {
		// TODO
	...
	protected void onRangeChanged(HasData<Node> display) {
		...
		navigationService.getNodesForPath(node.getPath(), new AsyncCallback<List<Node>>() {
			...
			public void onSuccess(List<Node> result) {
				...
				if (toIndex < 0) {
					...
					updateRowData(range.getStart(), Collections.<Node> emptyList());
					return;
				}
